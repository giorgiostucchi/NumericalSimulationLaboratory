...
//This program performs the integration with importance sampling
...
int main()
{
  ...
  string filename = "integrals&uncertainties2.txt";
  ofstream g;
  ...
  for (int i = 1; i <= N; i++)
  {
    ...
    for (int j = 0; j < L; j++)
    {
      sum_block += Integratore.IntegraDistribution(1000, f); // progressive sum within each block
    }
