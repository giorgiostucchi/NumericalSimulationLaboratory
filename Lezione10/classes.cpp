...
void CleanPreviousData(int option, int program)
{
    string program_string;
    if(program == 1) {
        program_string = "NonCommunicating";
    }else if(program != 1){
        program_string = "Parallel";
    };

    string option_string;
    if (option == 0)
        option_string = "circumference/";
    if (option == 1)
        option_string = "square/";
    if (option == 2)
        option_string = "AmericanCapitals/";
    string command = "rm -r " + program_string + option_string;
    int ret = system(command.c_str());
    if (ret != 0)
    {
        std::cerr << "Error: Couldn't delete folder " << ret << endl
                  << endl;
    }
}
...
void rnd_shuffle(vector<int>::iterator first, vector<int>::iterator last, Random &rnd)
{
    ...
    for (int i = n - 1; i > 0; --i)
    {
        swap(first[i], first[int(rnd.Rannyu(0, i + 1))]);
    }
}

void rnd_shuffle_complete(vector<int>::iterator first, vector<int>::iterator last, Random &rnd) // no element stays in the same position
{
    
    int n = (last - first);
    for (int i = n - 1; i > 0; --i)
    {
        int j = int(rnd.Rannyu() * (i + 1));
        if (j == i)
        {
            j--;
        }
...
Map::Map(int ncities, int opt, int program)
{
    m_ncities = ncities;
    m_opt = opt;
    m_program = program;
    ...
    else if (opt == 2)
    {
        ...
        double x, y;
        fstream input;
        input.open("American_capitals.dat", ios::in);
        if (!input.good())
        {
            cerr << "Could't load the capitals ... " << endl;
        }

        while (input >> state >> capital >> x >> y)
        {
            m_cities_coord.push_back(make_pair(x, y));
            linesread++;
        }
        input.close();
        m_ncities = linesread++;
        // cout << "american cities loaded" << endl;
    }
...
void Map::PrintMap(void)
{
    ...
    string filenamebest = program_string + option_string + "/" + "map.dat";
...
Population::Population(int individuals, Map &myMap) : m_map(myMap)
{
    ...
    int option = myMap.GetOption();
    int program = myMap.GetProgram();
    ...
    string option_string;
    if (option == 0)
        m_folder = program_string + "circumference/";
    if (option == 1)
        m_folder = program_string + "square/";
    if (option == 2)
        m_folder = program_string + "AmericanCapitals/";

    mkdir(m_folder.c_str(), 0777);
};

Population::Population(int individuals, Map &myMap, int skip) : m_map(myMap)
{
    string infile_primes = "Primes";
    string infile_seed = "seed.in";
    m_gen.SetSeed(infile_primes, infile_seed, skip);
...
void Population ::PrintFitness()
{
    ...
    string filenamebest = m_folder + "best_fitness.dat";
    string filenamebesthalf = m_folder + "besthalf_fitness.dat";
...
void Population ::PrintFitness(int rank)
{
    string rango = to_string(rank);
    string filenamebest = m_folder + "best_fitness_process_" + rango + ".dat";
    string filenamebesthalf = m_folder + "besthalf_fitness_process_" + rango + ".dat";
...
void Population ::PrintBestPath()
{
    ...
    string filenamebest = m_folder + "/BestPath.dat";
    ofstream best;
...
void Population ::PrintBestPath(int rank)
{
    string rango = to_string(rank);
    string filenamebest = m_folder + "/BestPath_process_" + rango + ".dat";
    ofstream best;
    ...
    best.close();
    cout << "Final Optimized Path printed for process number " << rank << " printed." << endl;
}

void Population ::PrintMap()
{
    string filenamebest = m_folder + "/map.dat";
    ofstream best;

    best.open(filenamebest, ios::app);
    for (int i = 0; i < m_map.GetNumberCities(); i++)
    {
        best << i << " " << m_map.GetNthCoordinates(i).first << " " << m_map.GetNthCoordinates(i).second << endl;
    }
    best.close();
...
void Population ::FullEvolution(int n_steps, int rank)
{
    for (int i = 0; i < n_steps; i++)
    {
