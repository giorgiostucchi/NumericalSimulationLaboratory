...
public:
  ...
  // Method to set the seed passing primes from the chosen file
  void SetSeed(string, string);
  // Method to set the seed passing primes from the chosen file, specifying the line to read from
  void SetSeed(string, string, int);
  ...
  // Method to generate a random number with a distribution -2(x-1)
  double Cumulative();
};
