...
void CleanPreviousData(int, int);
void rnd_shuffle(vector<int>::iterator first, vector<int>::iterator last, Random &rnd);
void rnd_shuffle_complete(vector<int>::iterator first, vector<int>::iterator last, Random &rnd);

class Map
{

public:
  Map(int ncities, int opt, int program);
...
public:
  ...
  void FullEvolution(int nsteps, int rank);
  void PrintFitness(void);
  void PrintFitness(int rank);
  double GetFitnessHalf(void);
  void PrintBestPath();
  void PrintMap();
  void PrintBestPath(int);
  void SetIndividual(int n, Individual ind) {m_population.at(n) = ind;};
