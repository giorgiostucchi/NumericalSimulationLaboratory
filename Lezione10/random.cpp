...
double Random ::Expo(double lambda = 1)
{
   return -1 / lambda * log(1 - Rannyu());
};

double Random ::Cauchy_Lorentz(double gamma = 1, double mu = 0)
{
...
void Random ::SetSeed(string filename, string fileinput)
{
   // This function sets the seed and parameters of the random number generator
   int seed[4];
   int p1, p2;
...
void Random::SetSeed(string filename, string fileinput, int skip)
{
   // This function sets the seed and parameters of the random number generator
   int seed[4];
   int p1, p2;
   ...
   if (Primes.is_open())
   {
      ...
      // Read the prime numbers from the desired line
      Primes >> p1 >> p2;
   }
   else
   {
   ...
   {
      cerr << "PROBLEM: Unable to open " << fileinput << endl;
   }
}
