...
int main()
{
  ...
  for (int index = 0; index <= n_equil; index++){Move();}; // include ciclo sui timestep per equilibrazione   2000 per solidi e liquidi, 100000 per gas
  int nconf = 1;
  ConfXYZ(nconf);
  nconf += 1;
...
void Input(void)
{
  ...
  ReadInput >> nphase;
  if(nphase == 0){n_equil = 2000; phase = "solid";}
  else if(nphase == 1){n_equil = 2000; phase = "liquid";}
  else{n_equil = 100000; phase = "gas";}
...
void Measure() // Properties measurement
{
  ...
  walker[ip] = rho * walker[it] + 48. * f / ((double)vol * 3.); // Total pressure;
  for (int i = 0; i < nbins; i++) walker_hist[i] = hist[i];
  return;
}
...
void Averages(int iblk) // Print results for current block
{
  ...
  ofstream Epot, Ekin, Etot, Temp, Pres, Rad, RadFinal;
  string mod;
  if(iNVET){mod = "_MC";}
  else{mod = "_NVE";}
  const int wd = 12;
  ...
  Epot.open("output_epot_" + phase + mod + ".dat", ios::app);
  Ekin.open("output_ekin_" + phase + mod + ".dat", ios::app);
  Temp.open("output_temp_" + phase + mod + ".dat", ios::app);
  Etot.open("output_etot_" + phase + mod + ".dat", ios::app);
  Pres.open("output_pres_" + phase + mod + ".dat", ios::app);
  Rad.open("output_rad_" + phase + mod + ".dat", ios::app);
  RadFinal.open("output_radfinal_" + phase + mod + ".dat", ios::app);
  ...
  for (int i=0; i<nbins; ++i)
{
        double r = (i + 0.5) * bin_size;
        double norm = 4.0 / 3.0 * pi * (pow(r + bin_size, 3) - pow(r, 3)) * rho * npart;
        stima_raddistr[i] = block_hist[i] / norm / blk_norm;
        av_raddistr[i] += stima_raddistr[i];
        av2_raddistr[i] += stima_raddistr[i] * stima_raddistr[i];
        err_raddistr[i] = Error(av_raddistr[i], av2_raddistr[i], iblk);
}
  ...
  Rad << setw(wd) << iblk << setw(wd);
  for(int i = 0; i < nbins ; i++) {Rad << stima_raddistr[i] <<setw(wd);} ;
  Rad << endl;
  if(iblk == nblk) {
    for(int i = 0; i < nbins ; i++) {RadFinal << av_raddistr[i] / (double)iblk << setw(wd) << err_raddistr[i] << endl;} ;
  }
...
{ // Write configuration in .xyz format
  ...
  WriteXYZ.open("/home/giorgiostucchi/LaboratorioSimulazioneNumerica/EserciziLaboratorio/Lezione4/MDNVE_MCNVT/frames/config_" + to_string(nconf) + ".xyz");
  WriteXYZ << npart << endl;
